...
func main() {
	...
	// client usage: go_grok [-h=<server hostname>] <local port>
	if flag.Arg(0) != "" {
		...
		fmt.Printf("port %s http available @:\n", flag.Arg(0))
	...
	// server usaage: go_grok [-h=<hostname>] [-b=<bind ip>]
	...
	log.Printf("go_grok server [%s] ready\n", *host)
	for {
